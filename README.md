...
11. [AI-asszisztált fejlesztés](#11-AI asszisztált fejlesztés)
...
| **Készlet bontás** | OnHand / Foglalt / Összes értékek külön oszlopokban |
| **Alkategória szűrő** | Az adott főkategórián belüli alkategóriák közötti gyors váltás (Tánccipő, Táncruha, Kiegészítők)|
| **Háttér betöltés** | A form indulása után csendben előtölti a kategóriákat a cache-be, így a kattintás után a termékek azonnal megjelennek |
| **Halasztott méret megnevezés betöltés** | A méretek megnevezését csak akkor kéri le, amikor kibontod a kártyát |

A háttérbetöltés és főleg a halasztott méret megnevezések betöltése mind a teljesítményt javítják. Az eddigi 12-15 másodperces beöltési idő kb. 6-ra csökkent abban az esetben ha először azonnal rányomunk egy kategória gombra, ha viszont többször nyomunk, vagy később nyomunk rá egy-egy kategóriára a form beöltését követően, akkor már gyorsabb lesz, mivel az alkalmazás betöltésekor már a chache-be tölti az adatokat. 
...
- **Halasztott betöltés (lazy loading)** — A kártya induláskor csak a bulk-művelethez *kötelező* adatokat kéri le (variánsok + inventory). A kozmetikai adatok (méret-feliratok, kategória-felirat) csak igény szerint érkeznek.
---

### Háttér előbetöltés
A form elindulása után 1-2 másodperccel egy csendes háttérművelet kezdi
előtölteni minden kategória termékadatait. Ez ~10-30 másodpercig tart
(adatállomány-függő), és a felhasználói interakciót nem akadályozza.
Bármelyik kategória kattintáskor:
- **Ha már előtöltött**: a kártyák azonnal megjelennek (< 1 mp).
- **Ha még folyamatban**: a kattintás és a háttér-feladat ugyanazt
  a Task-ot várja (nincs dupla hálózati hívás), és a kártyák a fetch
  befejezésekor jelennek meg.
...
### Típus-szűrő használata
- A kereső mellett jobbra látható a „Típus" legördülő.
- Tartalma a kattintott főkategória függvénye — Verseny vagy Amatőr
  alatt: Mind / Tánccipő / Táncruha; Kiegészítők alatt: csak Mind.
- A keresőszöveggel ÉS a típus-szűrővel együtt érvényesül (logikai ÉS).
...
### Készlet-oszlopok jelentése
A termékkártya tetején és a méret-panel minden sorában három érték
látható:
- **Készlet (On Hand)** — a raktárban fizikailag jelen lévő darabszám
- **Foglalt (Reserved)** — már megrendelt, de még ki nem szállított darabok
- **Eladható (Available)** = Készlet − Foglalt
...
class InventoryItemViewModel
{
    ...
    int TotalQuantityReserved // Összes variáns lefoglalt összege
    int TotalAvailable  // Az összes elérhető darabszám (Készlet - Lefoglalt)
...
## 11. Fejlesztői megjegyzések

- A `UiTheme.cs` az egyetlen forrása minden vizuális konstansnak — ha
  a kinézetet szeretnéd módosítani, ott kezd.
- Az `IHotcakesApiService` interfész lehetővé teszi a service kicserélését
  tesztelés vagy más API esetén.
- A halasztott opció-betöltést a `ProductListItem.EnsureOptionsLoadedAsync`
  kezeli; a háttér-kategóriabetöltést a `MainForm.FillCategoryLabelsInBackgroundAsync`.
- Új események: `ProductListItem.CategoryLoaded` (a háttér-betöltés után
  felhasználható a szűrő újrafuttatására).
...
**Claude Opus 4.7 (Claude App — Claude Code)**

Ugyanaz a Claude Code parancssori eszköz, mint a Sonnet 4.6 verzió,
de a háttérben az erősebb **Claude Opus 4.7** modell dolgozik. A „Claude
App" annyiban különbözik, hogy a CLI mellé egy egyszerű asztali burok
(natív ablak) is jár, amiben a beszélgetés könnyebben követhető,
viszont a működés ugyanaz — közvetlenül a projekt fájljain dolgozik.
